INSERT INTO public.cdn_scripts (slug, version, content, content_type, is_active, is_obfuscated)
VALUES (
  'bridge-button-beta.js',
  'v1.2.0-beta',
  $SCRIPT$// 🎯 BRIDGE COMMANDS: v1.2.0-beta - Interactive command popup for GHL chat (with Group Members)
...
(function() {
    ...
    // ── Upload helper for URL fields ──
    function wireUpload(uploadBtn, fileInput, textInput, cardIndex) {
    ...
    function renderBotoesForm(container) {
        ...
        function renderBtns() {
            ...
            btnsDiv.onclick = function(e) {
                if (e.target.hasAttribute('data-remove-botoes-btn')) removeBtn(parseInt(e.target.getAttribute('data-remove-botoes-btn')));
                else if (e.target.hasAttribute('data-add-botoes-btn')) addBtn();
            };
        }
        ...
        function buildBotoesCmd() {
            ...
            container.querySelector('[data-botoes-text]').style.borderColor = '#d1d5db';
            var btns = state.buttons.filter(function(b) { return b.label && b.label.trim(); }).map(function(b) {
            ...
            return '#botoes ' + text + (footer ? '|' + footer : '') + '|' + btns.join(',');
        }

        actionsDiv.querySelector('.bc-send-btn').addEventListener('click', function() {
            var cmd = buildBotoesCmd();
            if (cmd) { console.log(LOG, 'Sending:', cmd); if (sendCommand(cmd)) closePopup(); }
        });
        actionsDiv.querySelector('.bc-copy-btn').addEventListener('click', function() {
            var cmd = buildBotoesCmd();
            if (cmd) { navigator.clipboard.writeText(cmd).then(function() { var b = actionsDiv.querySelector('.bc-copy-btn'); b.innerHTML = '<span>✅</span> Copiado!'; setTimeout(function() { b.innerHTML = '<span>📋</span> Copiar'; }, 1500); }); }
        });
    }

    // ── Dynamic carousel form ──
    // (rest of script continues with renderCarouselForm, createPopup, renderCategory, inject, etc.)
    // The renderCategory function checks for customTab === 'members' to render the members tab

    function renderCarouselForm(container) {
        // ... carousel form rendering (same as original)
    }
        ...
        cat.commands.forEach(function(cmd, cmdIdx) {
            ...
            if (!cmd.fields || cmd.fields.length === 0) {
                ...
                card.querySelectorAll('.bc-upload-btn').forEach(function(btn) {
                    ...
                    wireUpload(btn, fileInput, textInput, 0);
                });
...
} catch(e) { console.error('Erro BridgeCmds:', e); }$SCRIPT$,
